...
| 실제 SNS 에서 수집한 영상 8건 (가짜 5, 진짜 3) | 가짜 5건 중 3건 탐지, 진짜 3건은 모두 바르게 판정. 표본이 작고 구성(한국인 3, 외국인 4, 다수 인물 1)이 고르지 않아 비율이 아닌 사례로 본다 |

같은 인물의 진짜 영상과 딥페이크 영상 한 쌍의 결과 화면이다. 진짜는 30점 안전으로 맞혔고, 가짜는 얼굴이 125 픽셀로 작아 14점 안전으로 놓쳤다. 놓친 쪽에는 입력 품질 경고가 붙는다. 같은 인물의 60초 실내 영상(진짜, 케이크 촛불을 부는 장면)은 37점 낮음으로 맞혔지만, 촛불을 부느라 입을 오므리고 눈을 감은 구간에서 CNN 이 50 % 까지 올라 신뢰도는 41 % 였다.
...
| **앙상블** | **6/8** | 가짜 2건 놓침, 오탐 0 |

앙상블은 어느 모델이 맞을지 미리 알 필요 없이 가장 좋은 단일 모델과 같은 정답을 냈고, 모델이 갈린 3건(가짜 1, 진짜 2)에서 모두 다수 쪽이 맞았다.
...
![같은 인물의 딥페이크 결과 — 14점, 안전 (놓침), 작은 얼굴 경고](docs/images/result_sample_fake.png)
...
- 학습 데이터(AI Hub, 변조 방법 5종, 스튜디오 촬영)와 환경이 다른 영상에서는 탐지율이 크게 떨어진다. 실제 SNS 딥페이크 영상 5건 중 3건을 탐지했고, 놓친 2건은 학습에 없는 생성기와 작은 얼굴이 원인이다.
...
- 학습 데이터 밖 평가가 영상 8건(가짜 5, 진짜 3)뿐이라 비율로 말할 수 없다. 학습 데이터의 인물은 전부 한국인인데 이 8건은 한국인 3, 외국인 4, 다수 인물 1로 섞여 있어 인종의 영향도 따로 잴 수 없다. 같은 인물(한국인)의 진짜·가짜 한 쌍에서는 진짜를 맞히고 가짜를 놓쳤다.
